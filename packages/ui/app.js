...
function connectProxyWebSocket() {
  ...
  socket.onmessage = (event) => {
    try {
      ...
      } else if (payload.type === "ai_suggestion") {
        handleAiSuggestion(payload.data);
...
let currentRepSpeaker = null;

window.markAsRep = function(speakerId) {
  currentRepSpeaker = speakerId;
  if (socket && socket.readyState === WebSocket.OPEN) {
    socket.send(JSON.stringify({ type: "set_rep_speaker", speaker_id: speakerId }));
  }
  // Remove buttons from UI
  document.querySelectorAll('.btn-mark-rep').forEach(btn => btn.style.display = 'none');
};
...
function handleFinalizedUtterance(utterance) {
  ...
  let btnHtml = "";
  if (currentRepSpeaker === null) {
    btnHtml = `<button class="btn-mark-rep" onclick="markAsRep(${utterance.speaker})" style="margin-left: 10px; font-size: 10px; cursor: pointer; background: #0284c7; color: white; border: none; padding: 2px 6px; border-radius: 4px;">Mark as Rep</button>`;
  } else if (currentRepSpeaker === utterance.speaker) {
    btnHtml = `<span style="margin-left: 10px; font-size: 10px; background: #28a745; color: white; padding: 2px 4px; border-radius: 4px;">You (Rep)</span>`;
  }

  div.innerHTML = `<strong style="color: #0284c7;">[Speaker ${utterance.speaker}]</strong>${btnHtml} ${utterance.text} <span style="color: #94a3b8; font-size: 11px; float: right;">${utterance.start.toFixed(1)}s - ${utterance.end.toFixed(1)}s</span>`;
...
function handleAiSuggestion(data) {
  ...
  div.style.borderRadius = "4px";
  div.style.border = "1px solid #f5d0fe";
  div.style.whiteSpace = "pre-wrap"; // Important to render \n as newlines
  
  div.innerHTML = `<strong style="color: #d946ef;">Suggestion:</strong><br/>${data.text}`;
